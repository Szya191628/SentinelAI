...
| 5 | 改造搜索执行流程 | 在搜索执行流程中串联关键词优化、工具查询、去重、聚类、情感分析 |
| 6 | 改造搜索结果承载结构 | 将情感分析摘要、聚类摘要写入 `current_search.metadata` 或 `DBResponse.parameters`，保证后续总结节点可读取 |
| 7 | 添加降级测试 | 验证模型不可用时主研究流程仍然可继续 |
...
情感分析和聚类不是独立节点，它们属于搜索增强流程。`initial_search` 和 `reflection_search` 仍然只负责生成搜索决策并调用搜索执行层；真正的关键词优化、工具查询、去重、聚类、情感分析都在搜索执行流程中完成。

```mermaid
flowchart TD
    A[搜索节点生成 SearchOutput] --> B[execute_search_and_convert]
    B --> C[搜索执行流程]
    C --> D{tool_name 类型}
    D -- search_hot_content --> E[直接查询热点内容]
    E --> F{情感分析开启?}
    F -- 是 --> G[对热点结果做情感分析]
    F -- 否 --> H[返回 DBResponse]
    G --> H
    D -- analyze_sentiment --> I[只做情感分析]
    I --> H
    D -- 其他本地搜索工具 --> J[关键词优化]
    J --> K[按优化关键词多次查询数据库]
    K --> L[合并结果]
    L --> M[按 URL 或内容去重]
    M --> N{ENABLE_CLUSTERING?}
    N -- 是 --> O[聚类采样]
    N -- 否 --> P[保留去重结果]
    O --> Q[构造 DBResponse]
    P --> Q
...
图中描述的是当前项目的真实执行逻辑。需要注意三类分支：

| 分支 | 处理逻辑 | 是否关键词优化 | 是否聚类 | 是否情感分析 |
| --- | --- | --- | --- | --- |
| `search_hot_content` | 直接查询热点内容，再按开关做情感分析 | 否 | 否 | 可选 |
| `analyze_sentiment` | 不查数据库，只对传入文本做情感分析 | 否 | 否 | 是 |
| 其他搜索工具 | 关键词优化、多次查询、合并、去重、聚类、情感分析 | 是 | 可选 | 可选 |

### 6.6 去重与聚类设计

聚类的目标不是生成新内容，而是减少大量相似搜索结果，让后续 LLM 总结时读到更少、更有代表性的材料。
...
| --- | --- |
| 1 | 遍历所有工具查询结果 |
| 2 | 如果结果有 `url`，使用 `url` 作为去重 key |
| 3 | 如果结果没有 `url`，使用 `title_or_content` 的前 100 个字符作为去重 key |
| 4 | 同一个 key 只保留第一条结果 |
...
| 步骤 | 规则 |
| --- | --- |
| 1 | 如果去重后结果数 `<= MAX_CLUSTERED_RESULTS`，不执行聚类，直接返回去重结果 |
| 2 | 如果结果数超过上限，取每条结果的 `title_or_content[:500]` 作为聚类文本 |
| 3 | 使用 `CLUSTERING_MODEL_NAME` 对文本生成 embedding |
| 4 | 使用 KMeans 聚类，聚类数约等于 `MAX_CLUSTERED_RESULTS / RESULTS_PER_CLUSTER` |
| 5 | 每个簇内部按 `hotness_score` 倒序排序 |
| 6 | 每个簇最多保留 `RESULTS_PER_CLUSTER` 条，直到总数达到 `MAX_CLUSTERED_RESULTS` |
| 7 | 聚类失败时返回前 `MAX_CLUSTERED_RESULTS` 条结果，主流程不中断 |

聚类前后数据变化：

| 阶段 | 数据形态 | 说明 |
| --- | --- | --- |
| 工具原始结果 | `list[QueryResult]` | 多个关键词、多张表查出来的原始结果，可能重复 |
| 去重后结果 | `list[QueryResult]` | 去掉相同 URL 或高度相似文本 |
| 聚类采样后结果 | `list[QueryResult]` | 数量被压缩，尽量保留不同主题和高热度材料 |
| 最终 `DBResponse.results` | `list[QueryResult]` | 后续 summary 节点真正读取的材料 |

当前项目中，聚类默认直接改变 `DBResponse.results` 和 `results_count`。为了让后续节点和测试能观察聚类效果，本阶段设计要求补充 `DBResponse.parameters["clustering"]` 或 `current_search.metadata["clustering"]`。
...
| --- | --- | --- |
| `enabled` | bool | 是否开启聚类 |
| `performed` | bool | 本次是否真的执行了聚类；结果数没超过上限时为 false |
| `original_count` | int | 工具合并后的原始数量 |
| `deduplicated_count` | int | 去重后的数量 |
| `sampled_count` | int | 最终返回给 LLM 的数量 |
| `max_results` | int | `MAX_CLUSTERED_RESULTS` 配置值 |
| `results_per_cluster` | int | `RESULTS_PER_CLUSTER` 配置值 |
| `fallback_reason` | string/null | 聚类失败或跳过的原因 |

### 6.7 情感分析设计

情感分析的目标是给搜索结果增加情绪分布信息，而不是替换搜索结果。搜索结果仍然放在 `DBResponse.results` 中，情感分析摘要挂在 `DBResponse.parameters["sentiment_analysis"]` 或 `current_search.metadata["sentiment_analysis"]` 中。

情感分析触发条件：

| 条件 | 说明 |
| --- | --- |
| 全局开关 | `SENTIMENT_ANALYSIS_ENABLED=true` 时才允许执行 |
| 单次搜索开关 | `enable_sentiment` 未传时使用 `ENABLE_SENTIMENT_PER_SEARCH` 默认值 |
| 结果非空 | 没有搜索结果时不执行批量情感分析 |
| 模型可用 | 缺少 PyTorch、Transformers 或模型加载失败时降级为未执行 |

情感分析处理流程：

```mermaid
flowchart TD
    A[DBResponse.results] --> B{情感分析是否开启}
    B -- 否 --> C[不写 sentiment_analysis]
    B -- 是 --> D{模型是否已初始化}
    D -- 否 --> E[尝试 initialize]
    D -- 是 --> F[抽取 QueryResult 文本]
    E --> F
    F --> G[调用 analyze_query_results]
    G --> H[统计情感分布和高置信度结果]
    H --> I[写入 sentiment_analysis]
```

图中描述的是情感分析的执行逻辑。系统先判断开关，再确保模型初始化。模型可用时，从 `QueryResult.title_or_content` 抽取文本，构造为 `content/platform/author/url/publish_time` 结构，再调用批量情感分析。模型不可用时不抛异常，而是返回“情感分析未执行”的透传结果。

情感分析摘要结构：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `total_analyzed` | int | 成功完成情感分析的文本数量 |
| `success_rate` | string | 成功数/总数，例如 `8/10` |
| `average_confidence` | float | 成功结果的平均置信度 |
| `sentiment_distribution` | object | 情感标签分布，例如 `{"负面": 6, "中性": 2}` |
| `high_confidence_results` | list | 置信度超过阈值的结果 |
| `summary` | string | 可直接给 summary 节点参考的情感摘要 |
| `available` | bool | 情感分析不可用时为 false，可选字段 |
| `reason` | string | 不可用或未执行的原因，可选字段 |

高置信度结果结构：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `original_data` | object | 原始查询结果的精简信息 |
| `sentiment` | string | 情感标签 |
| `confidence` | float | 置信度 |
| `text_preview` | string | 原文前 100 字符 |
...
```json
{
  ...
  "parameters": {
    "original_query": "某品牌售后争议",
    "optimized_keywords": ["某品牌售后", "退款问题", "客服投诉"],
    "optimization_reasoning": "这些关键词更贴近用户在社交平台上的表达",
    "sentiment_analysis": {
      "total_analyzed": 3,
      "success_rate": "3/3",
  ...
  "results": [
    {
      "platform": "weibo",
      "content_type": "comment",
      "title_or_content": "售后一直没有回应，体验很差",
      "author_nickname": "用户A",
      "url": null,
      "publish_time": "2026-05-19T10:00:00",
      "engagement": {
...
```json
{
  "sentiment_analysis": {
    "available": false,
    "reason": "缺少依赖: PyTorch / Transformers，情感分析已禁用。",
    "total_analyzed": 0,
    "success_rate": "0/3",
    "average_confidence": 0.0,
    "sentiment_distribution": {},
    "high_confidence_results": [],
    "summary": "情感分析未执行：缺少依赖"
  }
}
```

### 6.8 SearchResult Metadata 集成设计

当前搜索节点只把 `results` 写给 summary 节点还不够。第 06 步需要让 summary 节点也能拿到聚类和情感分析摘要。
...
| 位置 | 写入内容 |
| --- | --- |
| `research.current_search.results` | 聚类采样后的搜索结果 |
| `research.current_search.metadata.clustering` | 聚类执行摘要 |
| `research.current_search.metadata.sentiment_analysis` | 情感分析摘要 |
| `research.search_history[]` | 搜索词、工具、结果数量、时间戳；不需要保存完整情感分析明细 |

### 6.9 Summary 节点使用方式

| 节点 | 使用方式 |
| --- | --- |
| `initial_summary` | 使用 `current_search.results` 生成段落初稿；如果存在 `sentiment_analysis.summary`，在段落中补充整体情绪倾向 |
| `reflection_summary` | 使用补充搜索结果更新段落；如果情绪分布和上一轮不同，应说明变化 |
| `format_report` | 汇总各段落的情感摘要，可生成“公众情绪概览”小节 |

注意：summary 节点不能只依赖情感分析结论生成观点，仍然必须引用搜索结果中的具体文本。情感分析只能作为辅助信号。

### 6.10 注意事项

| 事项 | 要求 |
| --- | --- |
| 模型加载 | 情感分析和聚类模型应尽量懒加载；测试中必须避免 import 时触发真实模型下载或加载 |
| 缺少依赖 | 缺少模型依赖时降级，不影响 04 和 05 已完成的主流程 |
| 聚类顺序 | 必须先去重，再聚类，最后再做情感分析 |
| 热点工具例外 | `search_hot_content` 当前项目中不走关键词优化和聚类，只按开关做情感分析 |
| 测试边界 | 单元测试使用 FakeSentimentAnalyzer 和 FakeClusteringService |
| 性能边界 | 大量结果必须先聚类采样再送入 LLM |
| 结果可观测 | 聚类和情感分析的摘要必须写入 metadata，不能只在日志中出现 |

### 6.11 边界情况

| 场景 | 处理方式 |
| --- | --- |
| 情感模型下载失败 | 禁用情感分析，写入 `available=false` 和失败原因，主流程继续 |
| 情感依赖缺失 | 不执行模型推理，返回透传摘要 |
| 聚类模型不可用 | 返回前 `MAX_CLUSTERED_RESULTS` 条结果，并记录 `fallback_reason` |
| 文本为空 | 跳过或标记为分析失败 |
| 单条分析失败 | 记录失败项，批量继续 |
| 搜索结果少于上限 | 不执行聚类，直接返回去重结果 |
| 搜索结果过多 | 先去重，再聚类采样，再进入 summary 节点 |

### 6.12 测试用例设计

| 测试文件 | 测试目标 | Mock/前置条件 | 操作 | 预期结果 |
| --- | --- | --- | --- | --- |
| `tests/test_insight_engine_e2e.py` | 验证情感分析产物写入 | 使用 FakeSentimentAnalyzer | 调用搜索执行流程 | `DBResponse.parameters["sentiment_analysis"]` 存在，包含 `total_analyzed`、`sentiment_distribution`、`summary` |
...
| `tests/test_insight_engine_e2e.py` | 验证情感模型不可用降级 | FakeSentimentAnalyzer 标记不可用 | 执行研究链路 | 主流程继续，metadata 中记录 `available=false` 或失败原因 |
| `tests/test_insight_engine_e2e.py` | 验证聚类产物写入 | 使用 FakeClusteringService | 调用搜索执行流程 | metadata 中存在 `clustering`，包含 `original_count`、`deduplicated_count`、`sampled_count` |
...
| `tests/test_insight_engine_e2e.py` | 验证 summary 节点可读取 metadata | `current_search.metadata` 中准备情感和聚类摘要 | 执行 `initial_summary` 或 `reflection_summary` | LLM 输入中包含搜索结果和摘要信息 |
