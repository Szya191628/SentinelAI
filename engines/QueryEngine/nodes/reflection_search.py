...
class ReflectionSearchNode:
    ...
    def __call__(self, state: QueryGraphState) -> dict:
        ...
        reflection_input = {"title": para["title"], "content": para["content"], "paragraph_latest_state": para.get("research", {}).get("latest_summary", "")}
        raw = self.ctx.llm_client.stream_invoke_to_string(SYSTEM_PROMPT_REFLECTION, json.dumps(reflection_input, ensure_ascii=False))
    ...
    @staticmethod
    def _default() -> dict:
        return {"search_query": "深度研究补充信息", "search_tool": "basic_search_news", "reasoning": "默认反思搜索"}
